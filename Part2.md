...
- Component **definition** is how our component is declared.
  - A common good practice is to declcare a components' name with _Pascal Case_.
  - Hence if we're declaring a button component it should be `function Button() { ... }` instead of the usual _camelCase_ declaration.
...
- The **Body** of our component is where most of the `javascript` logic will be declared and take place.
  ...
  - Hence, we can declare _handlers_ to actually handle the components' internal logic.
...
The actual word `props` isnt' quite reserved unlike something like `const / let`. However it _does_ represent a good practice and up to a certain point, a standard in react components and the React community, which is why it's crucial to adhere to this convention for readability and maintainability of code. Remember, the objective is never to reinvent the wheel, but to make it faster and safer.

The word `props` refers to the properties of a component. These are all the attributes, data and information the component will need from its _parent_ component in order to be properly rendered.
...
In the previous snippet, **type, id && class** are the properties of the input tag. The use of this properties is irrelevant. What matters is that it helps the tag be _identified_ and _implemented_ properly. Hence, **props/properties**
...
1. Create a component called `Hero.jsx` inside your _Components_ directory. And write the code like so:
